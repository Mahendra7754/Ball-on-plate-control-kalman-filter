 ...
 */
 void KFcomputeFMat(FLOAT FMat[][NDIM], FLOAT FParams[]) {
    ...
    FLOAT kga = FParams[1];//*(3.14f/180); // Ball-plate gain
    FLOAT theta = FParams[2]; // xState(3) in deg
    FLOAT bias = FParams[3]; // xState(4) in degree
 ...
 */
 ...
 void KFpredictState(FLOAT xk[], FLOAT Phi[][NDIM], FLOAT G[], FLOAT uk) {
    FLOAT xpred[NDIM];
		int i, j;
		float sum;
    for (i = 0; i < NDIM; i++) {
...
*/
 ...
 void KF_PTUPD( FLOAT P[][NDIM], FLOAT Phi[][NDIM], FLOAT Q[] )
{
  ...
  for(i=0;i<NDIM;++i){
    for(j=0;j<NDIM;++j){
      s=0.0;
      for(k=0;k<NDIM;++k) s+= Phi[i][k]*P[k][j];
      T1[i][j]=s;
    }
  }
  for(i=0;i<NDIM;++i){
    for(j=0;j<NDIM;++j){
      s=0.0;
      for(k=0;k<NDIM;++k) s+= T1[i][k]*Phi[j][k]; // Phi'
      P[i][j]=s;
    }
    P[i][i]+=Q[i];
  }
 ...
 void KFcomputeKMat( FLOAT KMat[][MDIM], FLOAT PMat[][NDIM], FLOAT Rdiag[] )
{
  ...
  S = PMat[0][0] + Rdiag[0];
	invS = 1.0 / S;
  for( i=0;i<NDIM;++i) KMat[i][0] = PMat[i][0] * invS;
    // ...
...
{
	int i;
  for(i=0;i<NDIM;++i) KMat[i][0] = PMat[i][0] * CPCRinv[0][0]; // ...
 ...
 void KF_xMUPD( FLOAT xKF[NDIM], FLOAT KMat[][MDIM], FLOAT yMeas[MDIM] )
{
  // yhat = xKF[0]
	...
	int i;
 innov = yMeas[0] - xKF[0];
  for(i=0;i<NDIM;++i) xKF[i] += KMat[i][0]*innov;
