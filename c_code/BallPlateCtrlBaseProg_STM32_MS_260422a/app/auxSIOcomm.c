...
*  06.02.2017 mys   streamlining of checkReceivedOK( )
*  27.02.2017 mys   Eliminated state machine restarts via 0x0D payload bytes
*                   if (pRxBuf[1] == ASC_CR){rxState = XPOS_HI;} in serialReceive()
*  19.03.2026 mys   Y->xDotEstMS*PIXEL_PER_METER);    // [m/s] in [pxl/s]
*
*  @date    27/02/2017
**************************************************************************
*/
...
typedef enum RECEIVE_STATES      //!< state machine to handle auxSIO receive from PC
{
...
UINT16 receiveMeasurements( UINT8* pRxBuf )
/**
*  ***********************************************************************
...
*
*  @param  pRxBuf Uchar array with length 11 for received data
  ...
  do
  {
    switch( rxState )     // execute & propagate state machine states
    {
      case FRAME_START:
        pRxBuf[0] = _aux_getch();
        rxByteCount++;
        if(pRxBuf[0] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case XPOS_HI:
        ...
        pRxBuf[1] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[1] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case XPOS_LO:
        pRxBuf[2] = _aux_getch();
        rxByteCount++;
        //if( pRxBuf[2] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case YPOS_HI:
        pRxBuf[3] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[3] == ASC_CR) { rxState = XPOS_HI ;}
      ...
      case YPOS_LO:
        pRxBuf[4] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[4] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case XREF_HI:
        pRxBuf[5] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[5] == ASC_CR) { rxState = XPOS_HI ;}
      ...
      case XREF_LO:
        pRxBuf[6] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[6] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case YREF_HI:
        pRxBuf[7] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[7] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case YREF_LO:
        pRxBuf[8] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[8] == ASC_CR){ rxState = XPOS_HI ;}
      ...
      case CHECKSUM_HI:
        pRxBuf[9] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[9] == ASC_CR){rxState = XPOS_HI ;}
        //else {rxState = CHECKSUM_LO ;}
			  rxState = CHECKSUM_LO ;
        break;

      case CHECKSUM_LO:
        pRxBuf[10] = _aux_getch();
        rxByteCount++;
        //if (pRxBuf[10] == ASC_CR) {rxState = XPOS_HI ;}
...
UINT8  checkReceivedOK( UINT8* pPosMsgPC )
/**
*  ***********************************************************************
...
//CheckSumChk = ~CheckSumChk;
  ...
  {
    return 1; // checksum valid AND ball was detected   =>  NO ERROR
  }
  else
  {
    return 0; // checksum invalid OR ball not detected  =>  ERROR
  }
}


void sendLogData( const struct SS_VAR *X, const struct SS_VAR *Y )
/**
*  ***********************************************************************
*  @brief  Send logging data record to PC.
*  @param  X pointer to struct of type SS_VAR for x-axis
*  @param  Y pointer to struct of type SS_VAR for y-axis
**************************************************************************
*/
{
  sendLogFloat('a', X->xDotEstMS*PIXEL_PER_METER);    //11 Byte = 1 ID + 8 data bytes + 2 checksum bytes
  sendLogFloat('b', Y->xDotEstMS*PIXEL_PER_METER);    //11 Byte  [m/s] in [pxl/s]
  sendLogFloat('c', X->servoAngleCmdRad*CRAD2DEG);    //11 Byte = 1 ID + 8 data bytes + 2 checksum bytes
  sendLogFloat('d', Y->servoAngleCmdRad*CRAD2DEG);    //11 Byte
  sendLogShort('e', X->xPosMeasPxl);                  // 7 Byte = 1 ID + 4 data bytes + 2 checksum bytes
  sendLogShort('f', Y->xPosMeasPxl);                  // 7 Byte
  sendLogShort('g', X->xPosRefPxl);                   // 7 Byte
  sendLogShort('h', Y->xPosRefPxl);                   // 7 Byte
  sendLogFloat('i', X->plateAngleRad*CRAD2DEG);       //11 Byte     corr  Mys 171115
  sendLogFloat('j', Y->plateAngleRad*CRAD2DEG);       //11 Byte
  sendLogShort('k', X->servoAngleCmdPWM);             // 7 Byte
  sendLogShort('l', Y->servoAngleCmdPWM);             // 7 Byte
  sendLogFloat('m', X->ssvEst_k1[0]*PIXEL_PER_METER); //11 Byte   [m]   in [pxl]
  sendLogFloat('n', X->ssvEst_k1[1]*PIXEL_PER_METER); //11 Byte   [m/s] in [pxl/s]
  sendLogFloat('o', Y->ssvEst_k1[0]*PIXEL_PER_METER); //11 Byte         in [pxl]
  sendLogFloat('p', Y->ssvEst_k1[1]*PIXEL_PER_METER); //11 Byte         in [pxl/s]
                                                    // ---------------------------------------------
                                                    // 130 Byte logdata per processing cycle / frame
  _aux_putch(ASC_LF); // is ignored by PC
  _aux_putch(ASC_CR); // for a neat display (not HYPERTERMINAL)
}


void sendLogFloat( UINT8 msgID, float floatWORD )
/**
*  ***********************************************************************
*  @brief  Sends float value via auxSIO to PC.
*
*  Splits a FLOAT value into 4 bytes and generates a checksum
*  Furthermore splits every byte into upper and lower nibble (4 bit) and sends
*  them via auxSIO to PC.
*
...
void sendLogShort( UINT8 msgID, short Word16Bit )
/**
*  ***********************************************************************
*  @brief  Sends short int/16-Bit value via auxSIO to PC.
*
*  Splits int/16-Bit value into 2 bytes and generates a checksum
