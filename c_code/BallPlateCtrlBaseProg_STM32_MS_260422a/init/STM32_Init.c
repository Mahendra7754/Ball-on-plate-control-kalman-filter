 ...
 *----------------------------------------------------------------------------
 * History:
 *          160813 MYS changed several DEFINED SYMBOLS to be compatible with 
 *                 newer definitions in stm32f10x.h, coming with MDK_V4.71
 *                 e.g. TIMX_CR1_CEN -> TIM_CR1_CEN
 *                 e.g. TIM1234_IRQChannel -> TIM1234_IRQn
 *                 e.g. USART123_IRQChannel -> USART123_IRQn
...
#define __TIMER_USED              0x0004                  //  1
#define __TIMER_DETAILS           0x000C                  //  2
...
#define __USART3_REMAP            0x00000010
...
#define __GPIOA_CRL               0xBB008900
#define __GPIOA_CRH               0x00000890
...
#define __EFI_ACR_Val             0x00000032
...
//****NVIC->ISER[0] |= (1 << (TIM1_UP_IRQChannel & 0x1F));  // enable interrupt
      NVIC->ISER[0] |= (1 << (TIM1_UP_IRQn & 0x1F));        // enable interrupt MYS 160813 IRQn ! 
    }

    TIM1->CR1 |= TIM_CR1_CEN;                               // enable timer MYS 160813 TIMX_CR1_CEN !
  } // end TIM1 used
...
//****NVIC->ISER[0] |= (1 << (TIM2_IRQChannel & 0x1F));     // enable interrupt MYS
      NVIC->ISER[0] |= (1 << (TIM2_IRQn & 0x1F));           // enable interrupt MYS 160813 IRQn !
    }
...
//****NVIC->ISER[0] |= (1 << (TIM3_IRQChannel & 0x1F));     // enable interrupt
      NVIC->ISER[0] |= (1 << (TIM3_IRQn & 0x1F));           // enable interrupt MYS 160813 IRQn !
		}
...
//****NVIC->ISER[0] |= (1 << (TIM4_IRQChannel & 0x1F));     // enable interrupt
      NVIC->ISER[0] |= (1 << (TIM4_IRQn & 0x1F));           // enable interrupt MYS 160813 IRQn !
		}
...
#endif // GPIO Port F & G only on HD or XL devices
...
__inline static void stm32_UsartSetup (void) {
  ...
  if (__USART_USED & 0x01) {                                // USART1 used
    ...
    if (__USART_INTERRUPTS & 0x01) {                        // interrupts used
      ...
      NVIC->ISER[1] |= (1 << (USART1_IRQn & 0x1F));         // enable interrupt MYS 160813 IRQn !
    }
  ...
  if (__USART_USED & 0x02) {                                // USART2 used
    ...
    if (__USART_INTERRUPTS & 0x02) {                        // interrupts used
      ...
      NVIC->ISER[1] |= (1 << (USART2_IRQn & 0x1F));         // enable interrupt MYS 160813 IRQn !
    }
  ...
  if (__USART_USED & 0x04) {                                // USART3 used
    ...
    if (__USART_INTERRUPTS & 0x04) {                        // interrupts used
      ...
      NVIC->ISER[1] |= (1 << (USART3_IRQn & 0x1F));         // enable interrupt MYS 160813 IRQn !
    }
